...
{
    public class CAssetReferenceTreeModel : AssetTreeModel
    {
        public override void SetDataPaths(string refPathStr, string pathStr, string commonPathStr)
        {
            ...
            foreach(var item in resFileList)
            {
                if (item.StartsWith(refPaths[0]) || IsCommon(item) || item.EndsWith(".codesync"))
                {
                    continue;
                }
                res.Add(item);
        ...
        private bool IsCommon(string str)
        {
